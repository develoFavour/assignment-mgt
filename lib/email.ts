...
class BrevoEmailService {
    ...
    const htmlContent = `
      ...
      <head>
        ...
        <style>
          ...
          .title { font-size: 24px; font-weight: 800; color: #0f172a; margin: 0; }
          .subtitle { font-size: 16px; color: #64748b; margin-top: 5px; }
          .score-box { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 16px; padding: 30px; margin: 25px 0; text-align: center; color: white; }
          .score-value { font-size: 48px; font-weight: 900; margin: 10px 0; }
          .score-label { font-size: 14px; text-transform: uppercase; letter-spacing: 0.1em; opacity: 0.9; }
          .feedback-box { background: #fef3c7; border-left: 4px solid #f59e0b; padding: 16px; border-radius: 8px; margin: 20px 0; }
          .feedback-label { font-size: 12px; font-weight: 700; color: #92400e; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 8px; }
          .feedback-text { font-size: 14px; color: #78350f; font-style: italic; line-height: 1.6; }
          .button { display: inline-block; background: #0f172a; color: white; padding: 14px 28px; text-decoration: none; border-radius: 12px; font-weight: 600; margin-top: 10px; text-align: center; width: 100%; box-sizing: border-box; }
          .footer { text-align: center; margin-top: 30px; font-size: 12px; color: #94a3b8; }
        </style>
      </head>
      <body>
        <div class="container">
          <div class="card">
            <div class="header">
              <div class="icon">🎓</div>
              <h1 class="title">Your Assignment Has Been Graded</h1>
              <p class="subtitle">${courseName}</p>
            </div>
            
            <p>Hello,</p>
            <p>Your submission for <strong>${assignmentTitle}</strong> has been reviewed and graded by your instructor.</p>
            
            <div class="score-box">
              <div class="score-label">Your Score</div>
              <div class="score-value">${finalScore}/${totalMarks}</div>
              <div class="score-label">${((finalScore / totalMarks) * 100).toFixed(1)}%</div>
            </div>
            ...
            <a href="${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/student/assignments" class="button">View Full Grade Report</a>
          </div>
          <div class="footer">
            <p>Hallmark University • Academic Results</p>
          </div>
        </div>
      </body>
      </html>
  ...
  async sendNewMaterialEmail(
      ...
      <body>
        <div class="container">
          <div class="card">
            <div class="header">
              <div class="icon">📁</div>
              <h1 class="title">New Study Material Available</h1>
              <p class="subtitle">${courseName}</p>
            </div>
            
            <p>Hello,</p>
            <p>Your lecturer has uploaded new study material for your course. You can access it now on the portal.</p>
            
            <div class="info-box">
              <div class="info-item">
                <span class="label">Material Title</span>
                <div class="value">${materialTitle}</div>
              </div>
            </div>

            <a href="${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/student/materials" class="button">Access Materials</a>
          </div>
          <div class="footer">
            <p>Hallmark University • Academic Resources</p>
          </div>
        </div>
      </body>
      </html>
    ...
    const payload = {
      ...
      bcc: uniqueEmails.map(email => ({ email })),
      subject: `[New Material] ${courseName}: ${materialTitle}`,
      htmlContent,
    };
    ...
    try {
      ...
      console.error("Failed to send material notification emails:", error);
    }
  }
  ...
  async sendDeadlineReminderEmail(
    ...
    const htmlContent = `
      ...
      <head>
        ...
        <style>
          body { font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; line-height: 1.6; color: #1e293b; background-color: #f1f5f9; }
          .container { max-width: 600px; margin: 0 auto; padding: 20px; }
          .card { background: #ffffff; padding: 40px; border-radius: 16px; box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1); border-top: 4px solid #ef4444; }
          .header { text-align: center; margin-bottom: 30px; }
          .icon { font-size: 48px; margin-bottom: 10px; }
          .title { font-size: 24px; font-weight: 800; color: #0f172a; margin: 0; }
          .subtitle { font-size: 16px; color: #ef4444; font-weight: 700; margin-top: 5px; }
          .info-box { background: #fff1f2; border: 1px solid #fecaca; border-radius: 12px; padding: 20px; margin: 25px 0; }
          .info-item { margin-bottom: 15px; }
          .info-item:last-child { margin-bottom: 0; }
          .label { display: block; font-size: 11px; text-transform: uppercase; letter-spacing: 0.1em; color: #f87171; font-weight: 700; margin-bottom: 4px; }
          .value { font-size: 16px; font-weight: 600; color: #991b1b; }
          .button { display: inline-block; background: #ef4444; color: white; padding: 14px 28px; text-decoration: none; border-radius: 12px; font-weight: 600; margin-top: 10px; text-align: center; width: 100%; box-sizing: border-box; }
          .footer { text-align: center; margin-top: 30px; font-size: 12px; color: #94a3b8; }
        </style>
      </head>
      <body>
        <div class="container">
          <div class="card">
            <div class="header">
              ...
              <p class="subtitle">Only ${hoursRemaining} hours left</p>
            </div>
            
            <p>Hello,</p>
            <p>This is a friendly reminder that the deadline for your assignment <strong>${assignmentTitle}</strong> in <strong>${courseName}</strong> is approaching. We noticed you haven't submitted your work yet.</p>
            ...
            <p>Please ensure you upload your files before the deadline to avoid any late submission penalties.</p>

            <a href="${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/student/assignments" class="button">Submit Assignment Now</a>
          </div>
          <div class="footer">
            <p>Hallmark University • Academic Reminders</p>
          </div>
        </div>
      </body>
      </html>
    ...
    const payload = {
      ...
      to: [{ email: this.senderEmail }],
      bcc: uniqueEmails.map(email => ({ email })),
      subject: `[REMINDER] Deadline Approaching: ${assignmentTitle}`,
      htmlContent,
    };
    ...
    try {
      ...
      console.error("Failed to send deadline reminder emails:", error);
    }
  }
}
