		...
		try {
			const res = await fetch(
				`/api/lecturer/assignments?lecturerId=${session?.userId}`,
				{
					...
					body: JSON.stringify({
						...formData,
						total_marks: Number.isNaN(formData.total_marks) ? 100 : formData.total_marks,
						cutoff_days: Number.isNaN(formData.cutoff_days) ? 7 : formData.cutoff_days,
						penalty_percent: Number.isNaN(formData.penalty_percent) ? 10 : formData.penalty_percent,
							...
							<form onSubmit={handleAddAssignment} className="space-y-4">
								...
								<div>
									...
									<Input
										...
										value={Number.isNaN(formData.total_marks) ? "" : formData.total_marks}
										onChange={(e) =>
											setFormData({ ...formData, total_marks: e.target.value === "" ? NaN : Number.parseInt(e.target.value) })
										}
										required
									/>
								</div>
