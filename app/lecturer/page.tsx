		...
		const fetchStats = async () => {
			...
			try {
				const res = await fetch(
					`/api/lecturer/stats?lecturerId=${session.userId}`
				);
					...
					<CardHeader className="border-b border-border/50 pb-4">
						<div className="flex items-center justify-between">
							<div>
								...
								<CardDescription>
									Administrative actions for your courses
								</CardDescription>
							</div>
								...
								<div className="flex items-center gap-4">
									...
									<div>
										...
										<div className="text-sm text-muted-foreground">
											Draft and publish new academic tasks
										</div>
									</div>
								</div>
						...
						{loading ? (
							...
							<div className="space-y-6">
								{stats.recentSubmissions.map((sub) => (
									<div key={sub._id} className="flex items-start gap-3 group cursor-pointer" onClick={() => router.push('/lecturer/grades')}>
