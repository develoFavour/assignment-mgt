...
export default function AssignmentsPage() {
  ...
  const handleDelete = async (assignmentId: string) => {
    if (confirm("Are you sure you want to delete this assignment?")) {
      try {
        await fetch(`/api/lecturer/assignments/${assignmentId}`, { method: "DELETE" })
        ...
        <div className="p-6 space-y-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-muted-foreground mt-1">Create and manage your assignments</p>
            </div>
                  ...
                  <div className="space-y-3 border border-border rounded-lg p-4">
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        onChange={(e) => setFormData({ ...formData, accept_late: e.target.checked })}
                      />
                      <label htmlFor="accept_late" className="text-sm font-medium">
                    ...
                    {formData.accept_late && (
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <label className="text-sm font-medium">Cutoff Days</label>
                          <Input
                            type="number"
                            min="1"
                            value={formData.cutoff_days}
                            onChange={(e) => setFormData({ ...formData, cutoff_days: Number.parseInt(e.target.value) })}
                          />
                        </div>
                        <div>
                          ...
                          <Input
                            ...
                            onChange={(e) =>
                              setFormData({ ...formData, penalty_percent: Number.parseInt(e.target.value) })
                            }
                          />
                        </div>
                      </div>
          ...
          <Card>
            <CardContent className="pt-6">
              {loading ? (
                ...
                <div className="text-center text-muted-foreground py-8">No assignments yet</div>
              ) : (
                  ...
                  {assignments.map((assignment: any) => (
                    ...
                    >
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-muted-foreground mt-1">{assignment.course_code}</p>
