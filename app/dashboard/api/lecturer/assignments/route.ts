...
export async function GET(request: NextRequest) {
  try {
    ...
    const assignments = await assignmentsCollection.find({ created_by: new ObjectId(lecturerId) }).toArray()
...
export async function POST(request: NextRequest) {
  try {
    const { course_id, title, description, deadline, accept_late, cutoff_days, penalty_percent } = await request.json()
