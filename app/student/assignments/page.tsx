...
export default function AssignmentsPage() {
  ...
  useEffect(() => {
    const fetchAssignments = async () => {
      ...
      try {
        const res = await fetch(`/api/student/assignments?studentId=${session.userId}&filter=all`);
        // Fetch all initially, then client-side filter for speed/interactivity
        if (res.ok) {
  ...
  const filteredAssignments = useMemo(() => {
    return assignments.filter(assignment => {
      ...
      if (statusFilter === "submitted") return assignment.is_submitted && !assignment.is_graded;
  ...
  const getStatusBadge = (assignment: AssignmentWithStatus) => {
    ...
    return (
      <Badge variant="outline" className="text-slate-500 border-slate-200 font-bold px-3 py-1 rounded-full flex items-center gap-1.5">
      ...
      <main className={cn(
        ...
        <div className="max-w-5xl mx-auto space-y-8">
          ...
          <div className="space-y-1">
            ...
            <p className="text-slate-500 font-medium">Track deadlines and submit your academic work.</p>
          </div>
          ...
          <div className="space-y-4">
            {loading ? (
              <div className="flex flex-col items-center justify-center py-24 space-y-4">
                ...
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest animate-pulse">Loading Tasks...</p>
              </div>
              ...
              <div className="flex flex-col items-center justify-center py-24 text-center">
                ...
                <p className="text-slate-500 font-medium mt-1">No assignments found matching your filter.</p>
              </div>
            ) : (
                    ...
                    <Card className="h-full border-none shadow-sm hover:shadow-xl hover:shadow-primary/5 hover:-translate-y-1 transition-all duration-300 bg-white rounded-[24px] overflow-hidden">
                      <CardContent className="p-6 flex flex-col h-full">
                        ...
                        <div className="mt-6 pt-4 border-t border-slate-50 flex items-center justify-between">
                          {getDeadlineStatus(assignment.deadline, assignment.is_submitted)}
