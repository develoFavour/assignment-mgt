...
export default function AssignmentDetailPage() {
  ...
  useEffect(() => {
    const fetchDetailedData = async () => {
      ...
      try {
        const res = await fetch(
          `/api/student/assignments/${assignmentId}?studentId=${session.userId}`
        );
        if (res.ok) {
    ...
    if (session?.userId && assignmentId) {
      fetchDetailedData();
    }
  ...
  const handleSubmitWork = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/student/assignments/${assignmentId}/submit?studentId=${session?.userId}`,
        {
          method: "POST",
          body: formData,
        }
  ...
  const canSubmit =
    assignment &&
    !submission &&
    (!isDeadlinePassed || assignment.is_late_allowed);
                    ...
                    <div className="flex gap-3 p-4 rounded-xl bg-amber-50 text-amber-700 text-sm font-medium border border-amber-100/50">
                      ...
                      <div>
                        ...
                        <p className="text-xs opacity-90 mt-1">
                          You are passing the deadline. A{" "}
                          {assignment.penalty_percent}% grade penalty will apply
                          per day late.
                        </p>
                      </div>
                    </div>
          ...
          <div className="space-y-6">
            ...
            {grade && (
              <div className="bg-white rounded-[32px] p-6 shadow-sm border border-slate-100 relative overflow-hidden">
                ...
                <h3 className="text-sm font-black text-indigo-300 uppercase tracking-widest mb-4 relative z-10">
                  Grade Report
                </h3>

                <div className="text-center py-6 relative z-10">
                  <div className="text-5xl font-black text-indigo-600 tracking-tighter">
                    {grade.final_score}/{assignment?.total_marks || 100}
                  </div>
                  <p className="text-sm font-bold text-slate-400 mt-2">
                    {((grade.final_score / (assignment?.total_marks || 100)) * 100).toFixed(1)}%
                  </p>
                  {grade.penalty_applied > 0 && (
                    <p className="text-xs font-bold text-slate-400 mt-1">
                      {grade.score}/{assignment?.total_marks || 100} (Raw) - {grade.penalty_applied} penalty
                    </p>
                  )}
                </div>
